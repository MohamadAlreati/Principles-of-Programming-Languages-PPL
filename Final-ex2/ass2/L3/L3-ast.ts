...
added by me|  ( class ( <var>+ ) ( <binding>+ ) )    / ClassExp(fields:VarDecl[], methods:Binding[]))
...
export type ClassExp = {tag: "ClassExp"; fields: VarDecl[]; methods: Binding[]; } //added by me 
...
export const makeClassExp = (fields: VarDecl[], methods: Binding[]): ClassExp =>
    ({tag: "ClassExp", fields: fields, methods: methods});//added by me 
// L3
...
export const isClassExp = (x: any): x is ClassExp => x.tag === "ClassExp"; //added by me 
// L3
...
const parseClassExp = (fields: Sexp, methods: Sexp): Result<ClassExp> =>{
    if(isArray(fields) && allT(isString, fields) && isArray(methods)) {
        return mapv(unwrap(methods), (bindings: Binding[]) => 
             makeClassExp(map(makeVarDecl, fields), bindings)) }

    return makeFailure("Invalid class definition");}

const unwrap = (sexp: Sexp): Result<Binding[]> =>{
    if(isNonEmptyList<Sexp>(sexp) && isNonEmptyList<Sexp>(sexp[0]))
        return parseBinding(sexp[0]);
    return makeFailure("Invalid binding");}

const parseBinding = (sexp:Sexp[]): Result<List<Binding>> =>
    mapResult((m:any) => parseGoodBinding(m[0], m[1]), sexp);


const parseGoodBinding = (variable: Sexp, val: Sexp): Result<Binding> =>
    !isIdentifier(variable) ? makeFailure("First arg of binding must be an identifier") :
    mapv(parseL3CExp(val), (value: CExp) => makeBinding(variable, value));
...
const unparseClassExp = (exp: ClassExp): string =>  //niv======================
`(class (${map((v: VarDecl) => v.var, exp.fields).join(" ")}) (${map((b: Binding) => `(${b.var.var} ${unparseL3(b.val)})`, exp.methods).join(" ")}))`;
