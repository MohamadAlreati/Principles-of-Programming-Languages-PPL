...
import { isBinding, isCExp, isLetExp } from "./L3-ast";
...
import { isClosure, makeClosure, Closure, Value, isClass, makeClass, makeObject, Class, isObject, SExpValue,isSymbolSExp } from "./L3-value";
...
const L3applicativeEval = (exp: CExp, env: Env): Result<Value> =>
    ...
    isClassExp(exp)? evalClass(exp, env):       ////////added by me
...
const applyObject = (obj: Value, args: Value[], env: Env) :Result<Value> =>{
    if(isSymbolSExp(args[0]) && isObject(obj)){
        ...
        const found_method = obj.class.methods.find(Binding => Binding.var.var === meth_name);
        if(found_method === undefined)
            makeFailure(`Unrecognized method: ${meth_name}`);
        ...
        if(my_method !== undefined && isProcExp(my_method)){
            ...
            const body = renameExps(my_method.body); //the body of the method
            const litFieldsValue : CExp[] = map(valueToLitExp, (meth_args_vals).concat(obj.fieldsVal)); //The values that the obj gets 
            const stringFields: string[] = (my_method_args_str).concat(map((field:VarDecl) => field.var, obj.class.fields));    //the VarDecl string that needs to be substitute
            return evalSequence(substitute(body, stringFields, litFieldsValue), env); // Evaluate the method body
        }
        return makeFailure(`Unrecognized method: ${meth_name}`);
    }
    return makeFailure("wrong input");
}
